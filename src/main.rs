...
fn read_image_file(path: &str) -> Result<Vec<u8>, std::io::Error> {
    ...
    if requested_file.ends_with(".jpg") ||
        requested_file.ends_with(".png") ||
            requested_file.ends_with(".ttf") ||
            requested_file.ends_with(".ico")  {
                //TODO Favicons do not work! why is this?
                let contents: Vec<u8> = read_image_file(file_name)?;
